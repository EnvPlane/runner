...
func TestRunnerPostJSONClassifiesMissingRuntimeCredentialForRecovery(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: http.StatusUnauthorized,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body:       io.NopCloser(strings.NewReader(`{"error":"missing runtime auth credential: runner auth token is not configured"}`)),
			Request:    request,
		}, nil
	})}
	err := reportRunnerHeartbeat(context.Background(), runnerConfig{
...
func TestRunnerPostJSONClassifiesRuntimeIdentityReissueForAutomaticRecovery(t *testing.T) {
	for _, code := range []string{"fixture_identity_reissued", "same_cluster_identity_reissued"} {
		t.Run(code, func(t *testing.T) {
			client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
				return &http.Response{
					StatusCode: http.StatusUnauthorized,
					Header:     http.Header{"Content-Type": []string{"application/json"}},
					Body:       io.NopCloser(strings.NewReader(`{"error":"runner identity was reissued; retry registration","code":"` + code + `"}`)),
					Request:    request,
				}, nil
			})}
			err := reportRunnerHeartbeat(context.Background(), runnerConfig{
			...
			if err == nil || !isRunnerRuntimeIdentityReissuedError(err) {
				t.Fatalf("heartbeat error = %v, want runtime identity reissued", err)
			}
		})
	}
}

func TestPrepareRunnerRuntimeIdentityRecoveryClearsOnlyPersistedRuntimeAuth(t *testing.T) {
	...
	prepareRunnerRuntimeIdentityRecovery(runnerConfig{ProjectID: "fixture", RunnerID: "fixture-runner", RunnerAuthTokenFile: authPath}, slog.Default())
